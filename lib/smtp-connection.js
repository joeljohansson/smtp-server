...
class SMTPConnection extends EventEmitter {
    ...
    init() {
        ...
        this._setListeners(() => {
            ...
            if (this._server.options.argus) {
                //skip delay, early talker
    ...
    send(code, data) {
        ...
        if (this._socket && this._socket.writable) {
            if (!this._server.options.argus || [220, 221, 250, 421].includes(code)) {
                this._socket.write(payload + '\r\n');
            } else {
                // argus needs time
                setTimeout(() => this._socket.write(payload + '\r\n'), 100);
            }
    ...
    _onCommand(command, callback) {
        ...
        if (!this._server.options.argus && !this._ready) {
